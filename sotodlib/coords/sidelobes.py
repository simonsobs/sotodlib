...
class SidelobeCutter:
    def __init__(self, mask, objname, dtype=np.float32, rise_tol=1*utils.degree):
        self.mask = mask
        self.lmap = enmap.read_map(mask).astype(np.double)
        self.objname= objname
        self.sys_sotodlib  = "%s,up=hor" % np.char.capitalize([objname])[0]
    ...
    def make_cuts(self, obs):
        ...
        cuts = so3g.proj.RangesMatrix.from_mask(tod.astype(int))
        return cuts
