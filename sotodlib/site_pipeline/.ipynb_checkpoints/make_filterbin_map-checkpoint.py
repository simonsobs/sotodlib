...
def ptp_cuts(obs, signal_name='dsT', kurtosis_threshold=5):
    while True:
        ...
        if kurtosis_ptp < kurtosis_threshold:
            ...
            #print(f'dets:{obs.dets.count}, ptp_kurt: {kurtosis_ptp:.1f}')
    print(f'ptp dets: {obs.dets.count}')
...
#@profile
def filter_data(obs, remove_hwpss=True, get_stats=True):
    """ All the post-demodulation operations """
    ### Noise info
    if get_stats: 
        #dsT_orig = obs.dsT #.copy()
        #demodQ_orig = obs.demodQ #.copy()
        #demodU_orig = obs.demodU #.copy()
        obs.wrap('dsT_orig', obs.dsT, [(0,'dets'),(1,'samps')])
        obs.wrap('demodQ_orig', obs.demodQ, [(0,'dets'),(1,'samps')])
        obs.wrap('demodU_orig', obs.demodU, [(0,'dets'),(1,'samps')])
        #get_psd(obs)
        #get_inv_var(obs)
    ...
    #detrend_tod(obs, method='median', signal_name='dsT')
    #detrend_tod(obs, method='median', signal_name='demodQ')
    #detrend_tod(obs, method='median', signal_name='demodU')
...
def save_obs_nosig(obs, prefix, freq, wafer):
    ...
    save_dir = os.path.join(prefix, f'aman/')    
...
def make_depth1_map(context, obslist, shape, wcs, noise_model, comps="TQU", t0=0, dtype_tod=np.float32, dtype_map=np.float64, comm=mpi.COMM_WORLD, tag="", verbose=0, preprocess_config=None, split_labels=None, singlestream=False, det_in_out=False, det_left_right=False, det_upper_lower=False, site='so_sat3', recenter=None):
    ...
    for oi in range(len(obslist)):
        ...
        save_obs_nosig(obs, prefix='/home/sa5705/SO/ISO_satp3/outputs/', freq=band, wafer=detset) 
