...
###############################################################################
def rerun_analysis(bsa, iva2, R0_thresh=30e-3):
    """
...
def fit_dP_opts(bsa, iva, rmin=0.1, rmax=0.99):
    ...
    n = int(np.ceil(max([1.0 / rmin, 1.0 / (1.0 - rmax), 1000])))
    n = min([n, 100000])
    ...
    def func_bs_model(i_bias, dP_opt=0.0):
        ...
        irat = (1.0 - L0) / ((1.0 - L0) + (1.0 + L0) * R / Rsh)
    ...
    for ch in range(iva["nchans"]):
        try:
            ...
            logalpha = model_logalpha(r, logp0=logp0, p1=p1, p2=p2, logp3=logp3)
            alpha = np.exp(logalpha)
            popt, pcov = curve_fit(
                func_bs_model,
                np.atleast_1d(Ib_bs),
                np.atleast_1d(Irat_bs),
                p0=P_opt * 1e-2,
            )
            dP_opts[ch] = popt
