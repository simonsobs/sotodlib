...
@dataclass
class DetCalCfg:
    "Configuration object for the det_cal update script"
    ...
    num_processes: int = 1
    "Number of parallel processes used to process data"

    max_num_obs: Optional[int] = None
    "Maximum number of observations to process per ``run_update`` call"
    ...
    def __post_init__(self):
        ...
        self.index_path = os.path.join(self.root_dir, "det_cal.sqlite")
        self.h5_path = os.path.join(self.root_dir, "det_cal.h5")
        self.failed_obsid_cache = os.path.join(self.root_dir, "failed_obsids.yaml")


def setup_files(cfg: DetCalCfg):
    "Creates directories and databases if they do not exist"
    if not os.path.exists(cfg.index_path):
        scheme = core.metadata.ManifestScheme()
        scheme.add_exact_match("obs:obs_id")
        scheme.add_data_field("dataset")
        db = core.metadata.ManifestDb(scheme=scheme)
        db.to_file(cfg.index_path)
...
def get_cal_resset(cfg: DetCalCfg, obs_id):
    ...
    # Apply TES corrections if specified
    ...
    data = np.array([astuple(c) for c in cals], dtype=CalInfo.dtype())
    return data


@dataclass
class ProcessResult:
    """Result from worker function, containing processed calibration data"""
...
def run_update(cfg: Union[DetCalCfg, str]):
    """
    Runs update on det-cal database. See ``DetCalCfg`` for configuration options
    """

    if isinstance(cfg, str):
        with open(cfg, "r") as f:
            cfg = DetCalCfg(**yaml.safe_load(f))
    setup_files(cfg)

    # Get obsids to process
    ctx = core.Context(cfg.context_path)
    ...
    if cfg.max_num_obs is not None:
        obs_ids = obs_ids[: cfg.max_num_obs]
    logger.info("Processing %d obsids...", len(obs_ids))

    pb = tqdm(
        total=len(obs_ids),
        desc="Processing obsids",
    )

    def callback(res: ProcessResult):
        if not res.success:
            logger.error("Error processing obs_id: %s", res.obs_id)
