...
def get_parser(parser=None):
    ...
    parser.add_argument(
        "--preprocess_config", type=str, help="Preprocess configuration file"
    )
    ...
    parser.add_argument(
        ...
        "--ntod",
        type=int,
        help="Special case of `tods` above. Implemented as follows: [:ntod]",
    )
    ...
    parser.add_argument(
        "--cont", action="store_true", default=False, help="continue a run"
    )
    parser.add_argument("-@", "--center-at", type=str)
    parser.add_argument("-w", "--window", type=float)
    parser.add_argument(
        "--nmat-dir", type=str, help="Directory to where nmats are loaded from/saved to"
    )
    parser.add_argument(
        "--nmat-mode",
        type=str,
        help="How to build the noise matrix. 'build': Always build from tod. 'cache': Use if available in nmat-dir, otherwise build and save. 'load': Load from nmat-dir, error if missing. 'save': Build from tod and save.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-W", "--wafer", type=str, nargs="+", help="Detector wafer subset to map with"
    )
    parser.add_argument(
        "--freq", type=str, nargs="+", help="Frequency band to map with"
    )
    parser.add_argument(
        "-g",
        "--tasks-per-group",
        type=int,
        help="number of tasks per group. By default it is 1, but can be higher if you want more than one MPI job working on a depth-1 map, e.g. if you don't have enough memory for so many MPI jobs",
    )
    parser.add_argument(
        "--rhs", action="store_true", default=False, help="Save the rhs maps"
    )
    parser.add_argument(
        "--bin", action="store_true", default=False, help="Save the bin maps"
    )
    parser.add_argument(
        "--srcsamp",
        type=str,
        help="Path to mask file where True regions indicate where bright object mitigation should be applied. Mask is in equatorial coordinates. Not tiled, so should be low-res to not waste memory.",
    )
...
def make_depth1_map(
    ...
    mapmaker = mapmaking.MLMapmaker(
        signals, noise_model=noise_model, dtype=dtype_tod, verbose=verbose > 0
    )
    if comm.rank == 0:
    ...
    for step in mapmaker.solve(maxiter=niter):
        ...
        if comm.rank == 0:
            L.info(
                f"{tag} CG step {step.i:5d} {step.err:15.7e} {t2-t1:6.1f} {(t2-t1)/nobs_kept:6.3f}"
            )
        t1 = time.time()
    map_ = step.x[1]
    ...
    return bunch.Bunch(
        map=map_, ivar=ivar, tmap=tmap, signal=signal_map, t0=t0, bin=bin_
    )
...
def main(config_file, defaults=DEFAULTS, **args):
    ...
    mapcat_settings = {
        "database_type": args["mapcat_database_type"],
    ...
    if args["nmat"] == "uncorr":
        ...
        noise_model = mapmaking.NmatDetvecs(
            verbose=verbose > 1, downweight=[1e-4, 0.25, 0.50], window=args["window"]
        )
    else:
    ...
    for oi in range(comm_inter.rank, len(obskeys), comm_inter.size):
        ...
        if comm_intra.rank == 0:
            L.info(
                f"{tag} Proc period {pid:4d} dset {detset}:{band} @{t:.0f} dur {(periods[pid,1]-periods[pid,0])/3600:5.2f} h with {len(obslist):2d} obs"
            )
        try:
            ...
            my_costs = np.array(
                [
                    tod.samps.count
                    * len(mapmaking.find_usable_detectors(tod, maxcut=0.3))
                    for tod in my_tods
                ]
            )
            ...
            my_tods, my_inds, my_costs = [
                [a[vi] for vi in valid] for a in [my_tods, my_inds, my_costs]
            ]
        ...
        # past here.
        my_inds = all_inds[
            utils.equal_split(all_costs, comm_intra.size)[comm_intra.rank]
        ]
        ...
        try:
            ...
            mapdata = make_depth1_map(
                context,
                [obslist[ind] for ind in my_inds],
                subshape,
                subwcs,
                noise_model,
                L,
