...
def get_split_flags(aman, proc_aman=None, split_cfg=None):
    '''
    Function returning flags used for null splits consumed by the mapmaking and bundling codes.
    Fields labeled ``field_name_flag`` contain boolean masks and ``_avg`` are the mean
    ...
    split_aman = core.AxisManager(aman.dets)
