...
class ObsFileDb(common.DBObject):
    ...
    The `prefix` option is the path relative to which filenames in the
    database should be interpreted.  This only applies to relative
    filenames (those not starting with /).
    ...
    def from_file(cls, filename, prefix=None, fmt=None, force_new_db=True):
        ...
        return cls(map_file=conn, init_db=False, prefix=prefix)
    ...
    def get_obs(self):
        ...
        """
        with self.connection() as conn:
            c = conn.execute('select distinct obs_id from files')
            ret = [r[0] for r in c]
        return ret
    ...
    def get_detsets(self, obs_id):
        ...
        """
        with self.connection() as conn:
            c = conn.execute('select distinct detset from files '
                                'where obs_id=?', (obs_id,))
            ret = [r[0] for r in c]
        return ret
    ...
    def get_dets(self, detset):
        ...
        """
        with self.connection() as conn:
            c = conn.execute('select det from detsets where name=?', (detset,))
            ret = [r[0] for r in c]
        return ret
    ...
    def get_files(self, obs_id, detsets=None, prefix=None):
        ...
        with self.connection() as conn:
            if detsets is None:
                c = conn.execute('select detset, name, sample_start, sample_stop '
    ...
    def verify(self, prefix=None):
        ...
        with self.connection() as conn:
            c = conn.execute('select name, obs_id, detset, sample_start '
                                'from files')
            rows = []
            for r in c:
    ...
    def drop_obs(self, obs_id):
        ...
        with self.connection() as conn:
            c = conn.execute('select name from files where obs_id=?',
                                (obs_id,))
            affected_files = [os.path.join(self.prefix, r[0]) for r in c]
            # Drop them.
            conn.execute('delete from frame_offsets where file_name in '
                            '(select name from files where obs_id=?)',
                            (obs_id,))
            conn.execute('delete from files where obs_id=?',
                            (obs_id,))
            conn.commit()
    ...
    def drop_detset(self, detset):
        ...
        # What files does this affect?
        with self.connection() as conn:
            c = conn.execute('select name from files where detset=?',
                                (detset,))
            affected_files = [os.path.join(self.prefix, r[0]) for r in c]
            # Drop them.
            conn.execute('delete from frame_offsets where file_name in '
                            ...
                            (detset,))
            conn.execute('delete from files where detset=?',
                            (detset,))
            conn.commit()
        return affected_files
    ...
    def drop_incomplete(self):
        ...
        with self.connection() as conn:
            ...
            conn.execute('delete from detsets where name not in '
    ...
    def get_file_list(self, fout=None):
        ...
        """
        with self.connection() as conn:
            c = conn.execute('select name from files order by '
...
def diff_obsfiledbs(db_left, db_right, return_detail=False):
    ...
    if isinstance(db_right, str):
        db_right = ObsFileDb.from_file(db_right, force_new_db=False)
    ...
    # Check meta.
    with db_left.connection() as conn_left:
        with db_right.connection() as conn_right:
            cursors = [
                conn.execute(
                    'select * from meta order by param') for conn in [conn_left, conn_right]]
            ...
            foffs = [ResultSet.from_cursor(conn.execute(
                'select * from frame_offsets')) for conn in [conn_left, conn_right]]
...
def patch_obsfiledb(patch_data, target_db):
    ...
    for file_entry in patch_data['new_files']:
        target_db.add_obsfile(**file_entry, commit=True)

    for det_entry in patch_data['new_dets']:
        target_db.add_detset(det_entry['name'], [det_entry['det']], commit=True)
...
def main(args=None, parser=None):
    ...
    with db.connection() as conn:
        if args.mode == 'files':
            ...
            c = conn.execute('select name from files '
            ...
            if v == 1:
                ...
                conn.execute('insert or replace into meta (param,value) values (?,?)',
                                ('obsfiledb_version', 3))
