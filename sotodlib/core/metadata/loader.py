    ...
    def from_loadspec(self, load_params):
        ...
        Arguments:
          load_params: an index dictionary.
            contains the result of ManifestDb.match(request), where the request
            is generated by context, combined with
            obsdb.get(request['obs:obs_id'], add_prefix='obs'), which is all
            values in the obsdb for that observations (keys prefixed by 'obs:')

        Returns:
          A ResultSet or similar metadata object.
            returned object must have the function object.restrict_dets()
