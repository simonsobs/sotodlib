...
def get_inv_var(configs, aman, full, logger, uniform=False):
    ...
    if not uniform:
        ...
        if ipipe["signal"] == "demodQU":
            logger.info(
                "Using demodQU signal for inverse variance calculation")
            wnq = np.array(
                [
                    full.noiseQ_nofit.white_noise[idet == full.dets.vals][0]
                    for idet in aman.dets.vals
                ]
            )
            wnu = np.array(
                [
                    full.noiseU_nofit.white_noise[idet == full.dets.vals][0]
                    for idet in aman.dets.vals
                ]
            )
            ...
            logger.info(
                "Using only demodQ signal for inverse variance calculation")
            wn = np.array(
                [
                    full.noiseQ_nofit.white_noise[idet == full.dets.vals][0]
                    for idet in aman.dets.vals
            ...
            logger.info(
                "Using only demodU signal for inverse variance calculation")
            wn = np.array(
                [
                    full.noiseU_nofit.white_noise[idet == full.dets.vals][0]
                    for idet in aman.dets.vals
...
def planet_mapmake_single_obs(
    ...
    logger.info(
        f"[PID {os.getpid()}] Running, {obs_id}, {wafer_info}, starttime = {time.perf_counter()}"
    )
    ...
    if configs["mapmaking"]["fittod"]["process"]:
        ...
        if isinstance(configs["mapmaking"]["fittod"]["r_use"], str):
            configs["mapmaking"]["fittod"]["r_use"] = eval(
                configs["mapmaking"]["fittod"]["r_use"]
            )
        if isinstance(configs["mapmaking"]["fittod"]["r_use"], str):
            configs["mapmaking"]["fittod"]["r_fit"] = eval(
                configs["mapmaking"]["fittod"]["r_fit"]
            )
        if isinstance(configs["mapmaking"]["fittod"]["mask_res"], str):
            configs["mapmaking"]["fittod"]["mask_res"] = eval(
                configs["mapmaking"]["fittod"]["mask_res"]
            )
        execute_todfit(
            aman,
            dbpath=configs["mapmaking"]["fittod"].get("dbpath"),
            center_on=configs["mapmaking"]["map"].get("source"),
            Tsignal=configs["mapmaking"]["fittod"].get("Tsignal", "dsT"),
            flags=configs["mapmaking"]["fittod"].get("flags", None),
            centroid_position=(0, 0),
            r_use=configs["mapmaking"]["fittod"]["r_use"],
            r_fit=configs["mapmaking"]["fittod"]["r_fit"],
            res=configs["mapmaking"]["fittod"]["mask_res"],
            max_pix=4000000,
            detrend=configs["mapmaking"]["fittod"].get(
                "detrend_eachscan", True),
            npoly=configs["mapmaking"]["fittod"].get("npoly", 0),
            defl_model=configs["mapmaking"]["fittod"].get(
                "deflection_model", True),
            imask_use=None,
            imask_fit=None,
            logger=logger,
        )
    ...
    if configs["mapmaking"]["fittod"]["fitselection"]:
        ...
        numdet_b_fitsel = apply_todfit_selection(
            aman,
            toddbpath=configs["mapmaking"]["fittod"]["dbpath"],
    ...
    if configs["mapmaking"].get("map_PCA", False):  # default is False
        logger.info(
            "Replacing aman.signal, aman.dsT, aman.demodQ, aman.demodU with the calculated PCA modes"
        )
        for signame in ["signal", "dsT", "demodQ", "demodU"]:
            modes, weights = (
                aman[f"pca_model_{signame}"].modes,
                aman[f"pca_model_{signame}"].weights,
            )
            aman[signame] = weights @ modes
            aman[signame] = aman[signame].astype(np.float32)
        logger.info(
            "Finished replacing aman.signal, aman.dsT, aman.demodQ, aman.demodU with the calculated PCA mode signal"
        )

    # Make maps
    logger.debug(configs["mapmaking"]["map"]["coordinate"])
    if configs["mapmaking"]["map"]["coordinate"] in [
        "detector_center",
        "boresight_center",
    ]:
        logger.debug("instrument centered coordinate")
        dets_used, yc, xc, map_vars, coadd_map = make_instrument_center(
            aman, configs["mapmaking"], logger=logger
        )
    elif configs["mapmaking"]["map"]["coordinate"] in [
        "planet_horizon",
        "planet_equatorial",
    ]:
        logger.debug("planet centered coordinate")
        dets_used, yc, xc, map_vars, coadd_map = make_planet_center(
            aman, configs["mapmaking"], logger=logger
        )
    else:
        raise ValueError(
    ...
    if configs["mapmaking"]["fitmap"].get("process", False):
        try:
            fitr = fit_map(
                        coadd_map["map"],
                        fitthre=configs["fitmap"].get("fitthre", 1.2),
...
def make_planet_center(aman, config, logger, rot_q=None, debug=False, fits_name=None):
    ...
    Returns:
        ...
        result: coadded map including maps, hits, weight, and weigted_map (car).
    """
    obsinfo = get_obsinfo(aman)
    ...
    geom = enmap.geometry(
        pos=box, res=config["map"]["res"] * coords.DEG, proj=config["map"]["proj"]
    )
    ...
    if config["map"]["coordinate"] == "planet_horizon":
        ...
        raise ValueError(
            f"Unknown coordinate system: {config['map']['coordinate']}")
    ...
    if (
        config["deflection_correction"]["process"]
        & config["deflection_correction"]["wafer_base"]
    ):
        ...
        for i, idetid in enumerate(aman.det_info.det_id):
            try:
                if config["xieta_correction"]["process"]:
                    fitinfo = get_db_planettod(
                        config["fittod"]["dbpath"],
                        obs_id=iobsid,
                        freq_channel=iband,
                        wafer=iws,
                        detid=idetid,
                ...
                if (
                    config["deflection_correction"]["process"]
                    and not config["deflection_correction"]["wafer_base"]
                ):
                    fitinfo = get_db_planettod(
                        config["fittod"]["dbpath"],
                        obs_id=iobsid,
                ...
                defl_params.append((np.nan, np.nan))
        logger.info(
            f"Number of detectors is reduced based on todfit: {aman.dets.count} --> {len(np.where(bls)[0])}"
        )
    ...
    if config["map"]["coordinate"] == "planet_horizon":
        if config["map"].get("recv_coords"):
            ...
            rot = so3g.proj.quat.rotation_lonlat(0, 0) * so3g.proj.quat.euler(
                2, -aman.boresight.roll
            )
    ...
    if (
        ...
        for i, idetid in enumerate(aman.det_info.det_id):
            iaman = aman.restrict(
                "dets", aman.det_info.det_id == idetid, in_place=False
            )
            isight = copy.copy(
                sight
            )  # need to copy sight for each detector because it will be modified
            deflq = coords.helpers.get_deflected_quat(
                aman, params=defl_params[i])
            ...
            P = coords.P.for_tod(
                ...
                hwp=True,
                comps="TQU",
                cuts=iaman.flags[config["map"]["flags"]],
            )
            iresult = coords.demod.make_map(
                iaman,
                P=P,
                dsT=iaman[config["map"]["Tsignal"]],
                det_weights_demod=iaman.inv_var,
            )
            iresult["hit"] = P.to_map(
                tod=iaman,
                signal=np.ones(
                    iaman[config["map"]["Tsignal"]].shape, dtype=np.float32),
                det_weights=None,
                comps="T",
            )[0]
            ...
            if config["map"]["single_save"]:
                ...
                direeach = os.path.join(
                    config["map"]["save_dire"], f"each_det/{iobsid}/{idetid}/"
                )
                save_results(iresult, iobsid, iband, iws,
                             direeach, fits_name=fits_name)
        ...
        if config["map"]["single_save"]:
            ...
            for i, idetid in enumerate(aman.det_info.det_id):
                ...
                isight = copy.copy(
                    sight
                )  # might not need to copy because we use the same isight
                P = coords.P.for_tod(
                    iaman,
                    sight=isight,
                    geom=geom,
            ...
            P = coords.P.for_tod(
                ...
                hwp=True,
                comps="TQU",
                cuts=aman.flags[config["map"]["flags"]],
            )
            result = coords.demod.make_map(
                aman,
                P=P,
                dsT=aman[config["map"]["Tsignal"]],
                det_weights_demod=aman.inv_var,
            )
            result["hit"] = P.to_map(
                tod=aman,
                signal=np.ones(
                    aman[config["map"]["Tsignal"]].shape, dtype=np.float32),
                det_weights=None,
                comps="T",
            )[0]
    ...
    if config["map"].get("recenter", False):
        ...
        yc, xc = np.nan, np.nan
    save_results(
        result, iobsid, iband, iws, config["map"]["save_dire"], fits_name=fits_name
    )
    map_vars = get_map_variances(
        result["map"], minr=config["map"]["var_minr"], maxr=config["map"]["var_maxr"]
    )
    if debug:
        return aman
    else:
        return detids, yc, xc, map_vars, result


def make_instrument_center(
    aman, config, logger, rot_q=None, debug=False, fits_name=None
):
    ...
    for idetid in aman.det_info.det_id:
        ...
        isight = copy.copy(
            sight
        )  # need to copy sight for each detector because it is modified in make_instrument_center_single
        try:
            iresult = make_instrument_center_single(
                iaman,
                iobsid,
                iband,
                iws,
    ...
    if config.get("bootstrap", {}).get("process", False):
        ...
        for i in range(N_bootstrap):
            ...
            if config["bootstrap"].get("save_map", True):
                enmap.write_fits(
                    os.path.join(config["bootstrap"]
                                 ["save_dire"], fits_name_boot),
                    coadd_map,
                )
                np.save(
                    os.path.join(
                        config["bootstrap"]["save_dire"],
                        f"index_{iobsid}_{iband}_{iws}_boot_{i}.npy",
                    ),
                    ind,
                )
            if config["bootstrap"].get("fit_map", False):
                ifitr = fit_map(
                    coadd_map,
                    fitthre=config["bootstrap"].get("fitthre", 1.0),
                    sig_ran=config["bootstrap"].get("sig_ran", 0.8),
                )
                save_pkl(
                    ifitr,
                    os.path.join(
                        config["bootstrap"]["save_dire"],
                        f"fit_{iobsid}_{iband}_{iws}_boot_{i}.pkl",
                    ),
                )
    ...
    if debug:
        ...
        del results, azpl, elpl, sight
        gc.collect()
        return dets_used, yc, xc, map_vars, result


def make_instrument_center_single(
    aman, obsid, band, ws, idetid, azpl, elpl, sight, config, logger
):
    ...
    if config["map"]["coordinate"] == "boresight_center":
        ...
        raise ValueError(
            f"Unknown coordinate system: {config['map']['coordinate']}")
    P = coords.planets.get_single_instrument_P(
        aman,
        azpl,
        elpl,
        sight=sight,
        size=config["map"]["size"] * coords.DEG,
        res=config["map"]["res"] * coords.DEG,
        flags=aman.flags[config["map"]["flags"]],
        boresight_centered=boresight_center,
    )
    result = coords.demod.make_map(
        aman, P=P, dsT=aman[config["map"]["Tsignal"]
                            ], det_weights_demod=aman.inv_var
    )
    result["hit"] = P.to_map(
        tod=aman,
        signal=np.ones(aman[config["map"]["Tsignal"]].shape, dtype=np.float32),
        det_weights=None,
        comps="T",
    )[0]
    return result
...
def get_map_center(tmap):
    ...
    popt, _ = curve_fit(
        twoD_Gaussian, [x.ravel(), y.ravel()], tmap.ravel(), p0)
...
def recenter_map(result, yc, xc, **kwargs):
    ...
    for ikey in result.keys():
        new_result[ikey] = reproject.thumbnails(
            result[ikey], coords=np.asarray([yc, xc]), **kwargs
        )
    return new_result
...
def coadd_maps(results):
    ...
    deshit, dessummaps, desws, deswmaps = add_maps_all(
        carhits, carweights, carwmaps)
    ret = {"hit": deshit, "map": dessummaps,
           "weighted_map": deswmaps, "weight": desws}
    return ret
...
def exe_remove_weights(signal_map, weights_map, dest=None, eigentol=1e-4):
    ...
    Returns:
      dest: output map
    """
    inverse_weights_map = cal_inverse_weights(
        weights_map, dest=None, eigentol=eigentol)
    if dest is None:
        dest = np.zeros_like(signal_map)
    dest[:] = coords.helpers._apply_inverse_weights_map(
...
def cal_inverse_weights(weights_map, dest=None, eigentol=1e-4):
    ...
    dest[:] = coords.helpers._invert_weights_map(
...
def execute_todfit(
    ...
    if imask_use is None:
        imask_use = {
            "shape": "circle",
            "xyr": [centroid_position[0], centroid_position[1], r_use],
        }
    if imask_fit is None:
        imask_fit = {
            "shape": "circle",
            "xyr": [centroid_position[0], centroid_position[1], r_fit],
        }
    source_flag_use = tod_ops.flags.get_source_flags(
        aman,
        merge=False,
        overwrite=True,
        source_flags_name=source_flags_name_use,
        mask=imask_use,
        center_on=center_on,
        res=res,
        max_pix=max_pix,
    )
    source_flag_fit = tod_ops.flags.get_source_flags(
        aman,
        merge=False,
        overwrite=True,
        source_flags_name=source_flags_name_fit,
        mask=imask_fit,
        center_on=center_on,
        res=res,
        max_pix=max_pix,
    ...
    if flags is not None:
        ...
        imask_flag = ~aman.flags[
            flags
        ]  # because flags is regions we do not want to use
            ...
            try:
                if detrend:
                    ...
                    for j in range(len(fitrange)):
                        ...
                        iy = np.concatenate(
                            [isignal[islicebg1], isignal[islicebg2]])
                ...
                if defl_model:
                    ...
                    popt, pcov = curve_fit(
                        defl_fit_model, ixietatheta4fit, itod4fit, p0, sigma=ierr4fit
                    )
...
def defl_fit_model(
    xieta_theta, amplitude, xo, yo, sigma_x, sigma_y, theta, ph_def, ph_hwp
):
    ...
    xieta = (nxi, neta)
    beam = twoD_Gaussian_normalized(
        xieta, amplitude, xo, yo, sigma_x, sigma_y, theta, offset
    )
    return beam
...
def twoD_Gaussian_normalized(
    xy, amplitude, xo, yo, sigma_x, sigma_y, theta, offset=0, normalize=True
):
    """Normalized 2D elliptical Gaussian
    Args:
        xy: (x, y) in radian
    ...
    if normalize:
        g = offset + coeff ** (-1) * amplitude * np.exp(
            -(a * (x_dif**2) + 2 * b * (x_dif) * (y_dif) + c * ((y_dif) ** 2))
        )
    else:
...
def apply_todfit_selection(
    aman,
    toddbpath,
    thoreshold_path,
    logger,
    apply_selection=["amplitude", "xo", "yo",
                     "sigmax", "sigmay", "defla", "deflp"],
):
    ...
    """
    logger.info(
        f"Apply todfit based selection with paramters: {apply_selection}")
    iobsid, itele, iband, iws, _ = get_obsinfo(aman)
    logger.info(f"Read threshold")
    thresholds = load_select_threshold(
        thoreshold_path, itele, aman.obs_info.start_time, iband, iws
    )
    logger.info(f"get db")
    fit_database = get_db_planettod(
        toddbpath, obs_id=iobsid, freq_channel=iband, wafer=iws
    )
...
def load_select_threshold(
    path,
    sat,
    ts,
    band,
    ws,
    satp1_dt_thre=datetime.datetime(2025, 2, 27, tzinfo=datetime.timezone.utc),
):
...
def get_pwv(aman, dbinfo):
    try:
        ...
        dbinfo.pwv_p2p = -999
    try:
        _, pwvs_apex = load_apex_pwv_range(
            aman.timestamps[0], aman.timestamps[-1])
        if np.all(np.isnan(pwvs_apex)):
            ...
            dbinfo.pwv_apex_p2p = (
                np.nanmax(pwvs_apex) - np.nanmin(pwvs_apex)
            ) * 0.84 + 0.03
    except:
        dbinfo.pwv_apex = -999
        dbinfo.pwv_apex_std = -999
    ...
    if dbinfo.pwv_apex == -999:
        try:
            ...
            dbinfo.pwv_apex = (
                np.nanmean(pwvs_apex) * 0.84 + 0.03
                # Convert from APEX to corresponding PWV at SO site (CLASS radiometer).
            )
            dbinfo.pwv_apex_std = np.nanstd(pwvs_apex) * 0.84 + 0.03
            dbinfo.pwv_apex_p2p = (
                np.nanmax(pwvs_apex) - np.nanmin(pwvs_apex)
...
def make_planet_map_db(aman, source):
    ...
    dbinfo.f_hwp = float(
        ...
        / (aman.timestamps[-1] - aman.timestamps[0])
        / (2 * np.pi)
    )
    dbinfo.roll_angle = aman.obs_info.roll_center
    dbinfo.scan_speed = float(
        np.rad2deg(1)
        * np.median(np.abs(np.diff(aman.boresight.az)) / np.diff(aman.timestamps))
    )
    return dbinfo
...
def get_pwv_sync(aman):
    ...
    Args:
        aman: axismanager
    """
    hkaman = io.hk_utils.get_hkaman(
        start=float(aman.timestamps[0]),
        stop=float(aman.timestamps[-1]),
        config=None,
        alias=None,
        fields=[
            "site.env-radiometer-class.feeds.pwvs.pwv",
        ],
        data_dir="/scratch/gpfs/SIMONSOBS/so/tracked/data/site/hk",
    )
...
def get_apex_data(
    start_date=datetime.datetime(2024, 5, 19), end_date=datetime.datetime(2024, 5, 21)
):
    ...
    outdata = {"timestamps": [d.timestamp()
                              for d in data["dates"]], "pwv": data["pwv"]}
    return outdata
...
def get_pwv_apex_sync(aman):
    ...
    start_date = datetime.datetime.fromtimestamp(
        aman.timestamps[0], tz=datetime.UTC)
    end_date = datetime.datetime.fromtimestamp(
        aman.timestamps[-1], tz=datetime.UTC)
    request = requests.post(
        APEX_DATA_URL,
        data={
            "wdbo": "csv/download",
    ...
    ifl = (timestamps > aman.timestamps[0]) & (
        timestamps < aman.timestamps[-1])
    pwv = data["pwv"][ifl]
    return pwv
...
def hermite_2d_model(xy, nx, ny, xo, yo, sigma):
    ...
    coeff = 2 ** (nx + ny) * math.factorial(nx) * \
        math.factorial(ny) * np.pi * sigma**2
...
def fit_map(imapcar, fitthre=1, sig_ran=0.9, xflip=False, yflip=False):
    ...
    def twoD_Gaussian_offset_fixed(xy, amplitude, xo, yo, sigma_x, sigma_y, theta):
        ...
        return twoD_Gaussian_normalized(
            xy, amplitude, xo, yo, sigma_x, sigma_y, theta, offset
        )

    p0 = [np.max(iTmap), 0, 0, 20 / 60, 20 / 60, 0]
    popt, pcov = curve_fit(twoD_Gaussian_offset_fixed,
                           ixy, iTmap, p0, sigma=bgstdt)
    ...
    p0 = [1, 1, 1, 0.16]
    poptq, pcovq = curve_fit(T2P_leak_model_hermite_fixed,
                             ixy, iQmap, p0, sigma=bgstdq)
    ...
    p0 = [1, 1, 1, 0.16]
    poptu, pcovu = curve_fit(T2P_leak_model_hermite_fixed,
                             ixy, iUmap, p0, sigma=bgstdu)
...
def add_mapfit_info(dbinfo, fitr=None):
    if fitr is None:
        return
    else:
        dbinfo.amplitude = fitr["popt"][0]
        dbinfo.xo = fitr["popt"][1]
        dbinfo.yo = fitr["popt"][2]
        dbinfo.sigmax = fitr["popt"][3]
        dbinfo.sigmay = fitr["popt"][4]
        dbinfo.theta = fitr["popt"][5]

        tpeak = fitr["popt"][0]/(2*np.pi*fitr["popt"][3]*fitr["popt"][4])
        dbinfo.peak = tpeak
        dbinfo.mq = (fitr["poptq"][0] / (np.sqrt(np.pi) * fitr["poptq"][3]) ) /tpeak
        dbinfo.d0q = (fitr["poptq"][1] / (np.sqrt(np.pi) * fitr["poptq"][3]) ) /tpeak
        dbinfo.d1q = (fitr["poptq"][2] / (np.sqrt(np.pi) * fitr["poptq"][3]) ) /tpeak
        dbinfo.sigmaq = fitr["poptq"][3]
        dbinfo.mu = (fitr["poptu"][0] / (np.sqrt(np.pi) * fitr["poptu"][3]) ) /tpeak
        dbinfo.d0u = (fitr["poptu"][1] / (np.sqrt(np.pi) * fitr["poptu"][3]) ) /tpeak
        dbinfo.d1u = (fitr["poptu"][2] / (np.sqrt(np.pi) * fitr["poptu"][3]) ) /tpeak
        dbinfo.sigmau = fitr["poptu"][3]
        dbinfo.redchit = fitr["redchit"]
        dbinfo.redchiq = fitr["redchiq"]
        dbinfo.redchiu = fitr["redchiu"]
        return
...
def test1():
    ...
    planet_mapmake_single_obs(
        config_path, obs_id, dets, verbosity=3, debug=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="./examplet.yaml",
        help="Path to the configuration file",
    )
    parser.add_argument(
        "--obslist_path", type=str, default=None, help="Path to the obslist file"
    )
    parser.add_argument(
        "--verbosity", type=int, default=2, help="Number for logger verbosity"
    )
    parser.add_argument(
        "--test", action="store_true", help="perform test function or not"
    )
