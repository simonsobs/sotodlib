...
class ArrayZipper:
    ...
    def unzip(self, x):
        return x.reshape(self.shape).astype(self.dtype, copy=False)

    def dot(self, a, b):
        return (
            np.sum(a * b) if self.comm is None else self.comm.allreduce(np.sum(a * b))
        )
...
class MapZipper:
    ...
    def unzip(self, x):
        return enmap.ndmap(x.reshape(self.shape), self.wcs).astype(
    ...
    def dot(self, a, b):
        return (
            ...
            else putils.allreduce(np.sum(a * b), self.comm)
        )
...
class TileMapZipper:
    ...
    def unzip(self, x):
        return tilemap.TileMap(
            x.reshape(self.geo.pre + (-1,)).astype(self.dtype, copy=False), self.geo
        )
...
class MultiZipper:
    ...
    def zip(self, *objs):
        return np.concatenate(
            [zipper.zip(obj) for zipper, obj in zip(self.zippers, objs)]
        )
...
def inject_map(obs, map, recenter=None, interpol=None):
    ...
    if map.shape[0] not in [1, 2, 3]:
        raise ValueError(
            "Map to inject must have either 1, 2 or 3 components, corresponding to T, QU and TQU."
        )
...
def project_out(d, modes):
    return d - modes.T.dot(modes.dot(d))
...
def measure_power(d):
    return np.real(np.mean(d * np.conj(d), -1))
...
def makebins(edge_freqs, srate, nfreq, nmin=0, rfun=None, cap=True):
    ...
    if cap:
        binds = np.concatenate([[0], binds, [nfreq]])
...
def find_modes_jon(
    ft, bins, eig_lim=None, single_lim=0, skip_mean=False, verbose=False
):
    ndet = ft.shape[0]
...
def infer_comps(ncomp):
    return ["T", "QU", "TQU"][ncomp - 1]
...
def parse_recentering(desc):
    ...
    for ai, arg in enumerate(args):
        ...
        if ai == 0 and len(toks) == 1:
            ...
            raise ValueError(
                "parse_recentering wants key=value format, but got %s" % (arg)
            )
        # Handle the values
        if ":" in val:
...
def evaluate_recentering(info, ctime, geom=None, site=None, weather="typical"):
    ...
    def to_cel(lonlat, sys, ctime=None, site=None, weather=None):
        ...
        if sys == "cel" or sys == "equ":
            ...
            return so3g.proj.CelestialSightLine.az_el(
                ctime, lonlat[0], lonlat[1], site=site, weather=weather
            ).coords()[0, :2]
        else:
...
def recentering_to_quat_lonlat(p1, p2, pu):
    ...
    R = (
        quat.euler(2, ra2)
        * quat.euler(1, np.pi / 2 - dec2)
        * quat.euler(2, np.pi - rau2)
        * R
    )
...
def downsample_ranges(ranges, down):
    ...
    if isinstance(ranges, so3g.RangesInt32):
        return so3g.RangesInt32.from_array(
            downsample_ranges(ranges.ranges(), down), (ranges.count + down - 1) // down
        )
    oranges = ranges.copy()
...
def downsample_cut(cut, down):
    ...
    factor down."""
    return so3g.proj.ranges.RangesMatrix(
        [downsample_ranges(r, down) for r in cut.ranges]
    )
...
def downsample_obs(obs, down, skip_signal=False):
    ...
    for key in cut_keys:
        new_key = key.split(".")[-1]
        res.flags.wrap(
            new_key, downsample_cut(obs[key], down), [(0, "dets"), (1, "samps")]
        )
