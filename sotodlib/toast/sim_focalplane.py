...
def sim_wafer_detectors(
    ...
    if wprops["packing"] == "F":
        ...
        layout_A = rhombus_hex_layout(nrhombus, width, "", "", gap=gap, pol=pol_A)
        layout_B = rhombus_hex_layout(nrhombus, width, "", "", gap=gap, pol=pol_B)
    ...
    for px in range(npix):
        ...
        for b in bands:
            for (
                ...
                dprops["wafer_x_mm"] = float(x)
                dprops["wafer_y_mm"] = float(y)
                ...
                dprops["pol_ang_wafer"] = float(np.degrees(wafer_gamma - wafer_rot_rad))

                # Physical polarization orientation on wafer is always expressed
                # as 0 or 45.
                dprops["pol_orientation_wafer"] = float(dprops["pol_ang_wafer"] % 90)
                ...
                if center is not None:
                    dprops["quat"] = [
                        float(x)
                        for x in qa.mult(center, layout[pxstr]["quat"]).flatten()
                    ]
                    _, _, temp_gamma = quat_to_xieta(dprops["quat"])
                    dprops["gamma"] = float(np.degrees(temp_gamma))
                else:
                    dprops["quat"] = [float(x) for x in layout[pxstr]["quat"].flatten()]
                    dprops["gamma"] = float(np.degrees(wafer_gamma))
                ...
                dprops["readout_freq_GHz"] = float(readout_freq[doff])
...
def load_wafer_data(filename, array_name, sub_array_name=None):
    ...
    if sub_array_name is not None:
        ...
        for i in range(len(rs)):
            row = rs.rows[i]
            rs.rows[i] = (
                tuple([r.replace(array_name, sub_array_name) for r in row[:2]])
                + row[2:]
            )
            ...
            if "L" in sub_array_name:
                ...
                rs.rows[i] = (
                    ...
                    + rs.rows[i][2:bp]
                    + tuple([switches["U"][rs.rows[i][bp]]])
                    + rs.rows[i][bp + 1 :]
                )

    wafer = loader.convert_det_info(rs, rs["det_id"]).wafer
    return wafer
...
def load_wafer_detectors(
    ...
    for i, detname in enumerate(wafer.dets.vals):
        ...
        # coordinates.
        dprops["gamma"] = float(np.degrees(wafer.angle[i]))
        dprops["pol_ang"] = dprops["gamma"]

        # Polarization angle in the wafer coordinate system
        dprops["pol_ang_wafer"] = float(
            np.round(np.degrees(wafer.angle[i]) - poloff, 2)
        )
        ...
        if center is not None:
            temp_quat = qa.mult(center, quat).flatten()
            dprops["quat"] = [float(x) for x in temp_quat]
            # Update gamma angle to include the center rotation
            _, _, temp_gamma = quat_to_xieta(dprops["quat"])
            dprops["gamma"] = float(np.degrees(temp_gamma))
            dprops["pol_ang"] = dprops["gamma"]
        else:
            dprops["quat"] = [float(x) for x in quat.flatten()]
...
def sim_telescope_detectors(hw, tele, tube_slots=None, det_info=None, no_darks=False):
    ...
    if ntube == 1:
        ...
        for tindx, tube_slot in enumerate(tube_slots):
            ...
            if det_info_file is None:
                qwcenters = [
                    xieta_to_quat(-wradius, wradius / np.sqrt(3.0), 0.0),
                    xieta_to_quat(wradius, wradius / np.sqrt(3.0), 0.0),
                ...
                qwcenters = [
                    xieta_to_quat(-wradius / np.sqrt(3.0), wradius, 0.0),
                    xieta_to_quat(2.0 * wradius / np.sqrt(3.0), 0.0, 0.0),
                    xieta_to_quat(-wradius / np.sqrt(3.0), -wradius, 0.0),
                ]
        ...
        for d, props in alldets.items():
            props["quat"] = [float(x) for x in qa.mult(fp_rot, props["quat"])]
            # Update gamma angle to include this rotation
            _, _, temp_gamma = quat_to_xieta(props["quat"])
            props["gamma"] = float(np.degrees(temp_gamma))
