...
# Main interface function here is get_wafer_info, which computes SO MF
# and UHF wafer_info tables.  For LF, see function get_wafer_info_lf.
#
# There are some notable differences in the MF/UHF wafer_info columns:
#
# - Only in MF/UHF: rhombus, row, col, crossover.
# - Only in LF: pixel_num.
#
# Also note the det_id format for LF optical detectors is based on
# (band, pixel_num, pol) rather than (band, row, col, pol).
#

#: Frequency label remapping for SO LF detectors.
_LF_REMAP = {
    27: 30,
    39: 40,
}
...
def get_wafer_info(array_name, base_config={}, array_config={}, raw=False,
    ...
    """
    mode = array_config.get('mode', base_config.get('mode', 'classic'))
    if mode == 'classic':
        pass
    elif mode == 'LF':
        return get_wafer_info_lf(
            array_name, base_config=base_config, array_config=array_config,
            raw=raw, include_no_match=include_no_match)
    else:
        raise ValueError(f'get_wafer_info unable to deal with mode="{mode}"')
...
def get_wafer_info_lf(array_name, base_config={}, array_config={}, raw=False,
                      include_no_match=True):
    """Assemble the "wafer info" for a set of UFMs being read out as a
    single stream_id, which is typically done for LF arrays.  This is
    an adapted form of :func:`get_wafer_info`.
    ...
    Notes:
      There are several files contributing to the construction of the wafer
      info.  Each may be overridden in the array_config dict; the
      values can be relative to ``base_config['array_info_dir']`` or
      absolute.  See source code for default filenames.
    ...
    filenames = [
        os.path.join(base_dir, array_config.get(key, default)) for key, default in [
            ('umux_map_file', 'umux_32_map.csv'),
            ('copper_map_file', 'NIST_LF_UFM_res_map.csv'),
            ('bias_groups', f'{array_name}_bias_group_map.csv'),
        ]
    ]
    umux_map, copper_map, bias_group_map = map(_read_csv_cols, filenames)

    # Use the bias_group_map to find the mux_pos_to_band_files.
    all_arrays = sorted(list(set(bias_group_map['array_name'])))
    mux_pos_files = array_config.get('mux_pos_to_band_files', {})
    mux_pos_to_band = []

    for array_name in all_arrays:
        default = f'{array_name}_mux_pos_num_to_mux_band_num.csv'
        filename = os.path.join(base_dir, mux_pos_files.get(array_name, default))
        this_mux_pos = _read_csv_cols(filename)
        _table_tag(this_mux_pos, 'array', array_name)
        _table_tag(this_mux_pos, 'is_north', 'True')
        mux_pos_to_band.append(this_mux_pos)

    # Convert from list of tables to single table.
    mux_pos_to_band = _table_stack(*mux_pos_to_band)

    # Extract bandpass value from 'tes_id' field (e.g. 27A, 39D ...)
    copper_map['bandpass'], copper_map['pol'] = [], []
    for tes_id in copper_map['tes_id']:
        bp, pol = 'NC', 'X'
        if tes_id != '':
            bp, pol = tes_id[:-1], tes_id[-1]
            # Check for validity.
            assert pol in ['A', 'B', 'D']
            int(bp)
        copper_map['bandpass'].append(bp)
        copper_map['pol'].append(pol)
    for k in ['bandpass', 'pol']:
        copper_map[k] = np.array(copper_map[k])

    # Remap the bandpass field, e.g. 27 -> 30 or whatever.
    bandpass_remap = base_config.get('bandpass_remap')
    if bandpass_remap is None:
        bandpass_remap = _LF_REMAP
    if bandpass_remap is not None:
        # Do it in two steps to support swapping, shrug.
        masks = []
        for k, v in bandpass_remap.items():
            masks.append((str(v), copper_map['bandpass'] == str(k)))
        for v, s in masks:
            copper_map['bandpass'][s] = v

    # The umux_map contains all muxable frequencies from 4-8 GHz.  But
    # SO uses 2 chips, and only the 4-6 GHz band from each.  Therefore
    # reformulate umux_map with a new column 'is_north', and repeat
    # the 4-6 GHz band.  For LF, for now, only one chip is used and we
    # declare it to be called North.
    s = np.array(list(map(int, umux_map['mux_band']))) < 14
    umux_map = {k: v[s] for k, v in umux_map.items()}
    umux_map['is_north'] = np.ones(s.sum(), bool).astype('str')

    # The various data tables are represented as dicts of arrays of
    # strings.  Strings are ok for now since it's mostly straight
    # table joins on equal values. Re-typing is dealt with in final
    # processing.
    ...
    for umux_i in range(len(umux_map['mux_band'])):
        ...
        # Use the array/mux_pos_num to look up detector details.
        this_ptb = _row_from_cols(mux_pos_to_band, mux_map_i)

        copper_i, bg_map_i = None, None
        this_copper, this_bg_map = {}, {}

        copper_i = _find_in_cols(copper_map,
                                 {'mux_layout_position': this_ptb['mux_pos_num'],
                                  'mux_bondpad': this_pad})

        if copper_i is not None:
            this_copper = _row_from_cols(copper_map, copper_i)
            bg_map_i = _find_in_cols(bias_group_map,
                                     {'array_name': this_ptb['array'],
                                      'array_bias_group': this_copper['bias_group']})

            this_bg_map = _row_from_cols(bias_group_map, bg_map_i)

        idx.append((umux_i, mux_map_i, copper_i, bg_map_i))
    ...
    if raw:
        return {
            'filenames': filenames,
            'table_names': ['umux', 'pos_to_band', 'copper', 'bg_map'],
            'tables': [umux_map, mux_pos_to_band, copper_map, bias_group_map],
            'indices': idx,
            'rows': data_rows,
        }
    ...
    output = []
    for row in data_rows:
        output.append(_process_row_lf(row))

    if include_no_match:
        output.append(_process_row_lf(
            [{'det_id': 'NO_MATCH', 'array_name': ''},{},{},{}]))

    return output


def _process_row_lf(row):
    # Organize and reformat all the data loaded for a resonator /
    # readout element into a wafer_info entry dict.  This is
    # specialized for use with get_wafer_info_lf results.
    #
    # To get a "NO_MATCH" row pass in
    #   ({'det_id': 'NO_MATCH', 'array_name': ...}, {}, {}, {}).

    umux, pos_to_band, copper, bg_map = row
    def _get(key, cast=lambda x: x, default=None, translate={}):
        for src in [umux, copper, bg_map, pos_to_band]:
            if key in src:
                v = src[key]
                v = translate.get(v, v)
                return cast(v)
    ...
    output = {ko: _get(ki, *args) for ko, ki, *args in [
        ...
        ('mux_position', 'mux_pos_num', int, -1),
        ('design_freq_mhz', 'freq_mhz', float, np.nan),
        ('bias_line', 'smurf_bias_group', int, -1, {'': -1}),
        ('pol', 'pol', str, 'X'),
        ('bandpass', 'bandpass', str, 'NC'),
        ('pixel_num', 'pixel_num', int, -1, {'': '-1'}),
        ('type', 'PLACEHOLDER'),
        ('x', 'x_mm', float, np.nan, {'': 'nan'}),
        ('y', 'y_mm', float, np.nan, {'': 'nan'}),
        ('angle', 'angle_actual_deg', float, np.nan, {'': 'nan'}),
        ('coax', 'is_north', str, None),
    ]}
    ...
    if output['det_id'] == 'NO_MATCH':
        label = 'NO_MATCH'
        det_type = 'NC'
    elif output['pixel_num'] < 0 or output['pol'] == 'D' or output['bandpass'] == 'NC':
        b, det_type = '%02i' % output['bond_pad'], 'SLOT'
        if b == '64':
            det_type = 'SQID'
        elif b == '-1':
...
#
# Support functions for working with dict-based tables...
#

def _table_tag(table, key, value):
    v = list(table.values())[0]
    table[key] = np.array([value] * len(v))

def _table_stack(*tables):
    keys = tables[0].keys()
    for t in tables[1:]:
        if set(t.keys()) != set(keys):
            raise ValueError(f"Tables have different fields -- {keys} vs. {t.keys()}")
    table_out = {}
    for k in keys:
        table_out[k] = np.hstack([t[k] for t in tables])
    return table_out
