...
from sotodlib.core.flagman import (
    has_any_cuts,
    has_all_cut,
    count_cuts,
...
class DetBiasFlags(_Preprocess):
    ...
    def calc_and_save(self, aman, proc_aman):
        msk = tod_ops.flags.get_det_bias_flags(aman, merge=False, **self.calc_cfgs)
...
class Trends(_Preprocess):
    ...
    def calc_and_save(self, aman, proc_aman):
        trend_cut, trend_aman = tod_ops.flags.get_trending_flags(
            aman, merge=False, full_output=True, **self.calc_cfgs
        )
    ...
    def select(self, meta):
        ...
        if self.select_cfgs["kind"] == "any":
            ...
            raise ValueError(
                f"Entry '{self.select_cfgs['kind']}' not"
...
class Jumps(_Preprocess):
    ...
    All calculation config go to `find_jumps`
    ...
    def process(self, aman, proc_aman):
        aman.signal = tod_ops.jumps.jumpfix_subtract_heights(
            aman.signal,
            proc_aman.jumps.jump_flag.mask(),
            inplace=True,
            heights=proc_aman.jumps.jump_heights,
        )

    def calc_and_save(self, aman, proc_aman):
        jumps, heights = tod_ops.jumps.find_jumps(aman, merge=False, **self.calc_cfgs)
        jump_aman = tod_ops.jumps.jumps_aman(aman, jumps, heights)
        aman.wrap(self.name, jump_aman)
    ...
    def save(self, proc_aman, jump_aman):
        if self.save_cfgs is None:
            return
        if self.save_cfgs:
            proc_aman.wrap(self.name, jump_aman)

    def select(self, meta):
        if self.select_cfgs is None:
            return meta

        n_cut = count_cuts(meta.preprocess.jumps.jump_flag)
        keep = n_cut <= self.select_cfgs["max_n_jumps"]
        meta.restrict("dets", meta.dets.vals[keep])
        return meta


class JumpsTwoPi(_Preprocess):
    """Run 2oi jump finding and fixing algorithm.
    All calculation config go to `twopi_jumps`

    Saves results in proc_aman under the "jumps_twopi" field.

    Data section should define a maximum number of jumps "max_n_jumps".

    .. autofunction:: sotodlib.tod_ops.jumps.twopi_jumps
    """
    ...
    def process(self, aman, proc_aman):
        aman.signal = tod_ops.jumps.jumpfix_subtract_heights(
            aman.signal,
            proc_aman.jumps_twopi.jump_flag.mask(),
            inplace=True,
            heights=proc_aman.jumps_twopi.jump_heights,
        )

    def calc_and_save(self, aman, proc_aman):
        jumps, heights = tod_ops.jumps.twopi_jumps(aman, merge=False, **self.calc_cfgs)
        jump_aman = tod_ops.jumps.jumps_aman(aman, jumps, heights)
        aman.wrap(self.name, jump_aman)
        self.save(proc_aman, jump_aman)
    ...
    def select(self, meta):
        ...
        n_cut = count_cuts(meta.preprocess.jumps_twopi.jump_flag)
        keep = n_cut <= self.select_cfgs["max_n_jumps"]
        meta.restrict("dets", meta.dets.vals[keep])
        return meta


class JumpsSlow(_Preprocess):
    """Run slow jump finding and fixing algorithm.
    All calculation config go to `slow_jumps`

    Saves results in proc_aman under the "jumps_slow" field.

    Data section should define a maximum number of jumps "max_n_jumps".

    .. autofunction:: sotodlib.tod_ops.jumps.slow_jumps
    """
    ...
    def process(self, aman, proc_aman):
        aman.signal = tod_ops.jumps.jumpfix_subtract_heights(
            aman.signal,
            proc_aman.jumps_slow.jump_flag.mask(),
            inplace=True,
            heights=proc_aman.jumps_slow.jump_heights,
        )

    def calc_and_save(self, aman, proc_aman):
        jumps, heights = tod_ops.jumps.slow_jumps(aman, merge=False, **self.calc_cfgs)
        jump_aman = tod_ops.jumps.jumps_aman(aman, jumps, heights)
        aman.wrap(self.name, jump_aman)
        self.save(proc_aman, jump_aman)
    ...
    def select(self, meta):
        ...
        n_cut = count_cuts(meta.preprocess.jumps_slow.jump_flag)
        keep = n_cut <= self.select_cfgs["max_n_jumps"]
        meta.restrict("dets", meta.dets.vals[keep])
        return meta
...
class PSDCalc(_Preprocess):
    ...
    def process(self, aman, proc_aman):
        ...
        fft_aman = core.AxisManager(aman.dets, core.OffsetAxis("fsamps", len(freqs)))
...
class Noise(_Preprocess):
    ...
    def calc_and_save(self, aman, proc_aman):
        ...
        wn = tod_ops.fft_ops.calc_wn(
            aman, pxx=aman.psd.Pxx, freqs=aman.psd.freqs, **self.calc_cfgs
        )
...
class Calibrate(_Preprocess):
    ...
    def process(self, aman, proc_aman):
        if self.process_cfgs["kind"] == "single_value":
            ...
            raise ValueError(f"Entry '{self.process_cfgs['kind']}'" " not understood")
