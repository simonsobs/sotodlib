...
def create_error_db(configs, obs_errors, exclude_no_error=True):
    """Create a ManifestDb containing entries for obs_ids, groups and errors
    from PreprocessErrors.

     Arguments
    ----------
    configs : dict
        The preprocessing config dictionary.
    obs_errors : dict
        Dictionary generated by preprocess_tod and multilayer_preprocess_tod
        that stores for each obs_id a list of dicts including the group and
        error.
    exclude_no_errors : bool
        If True, exclude entries whose errors are None, otherwise include them.
    """

    group_by = np.atleast_1d(configs['subobs'].get('use', 'detset'))

    err_db_path = os.path.join(os.path.dirname(configs["archive"]["index"]),
    ...
    if os.path.exists(err_db_path):
        ...
        for gb in group_by:
            scheme.add_exact_match('dets:' + gb)
        scheme.add_data_field('error')

        err_db = core.metadata.ManifestDb(
            os.path.join(err_db_path),
            scheme=scheme
        )

    for obs_id, out_meta in obs_errors.items():
        for i in range(len(out_meta)):
            if exclude_no_error and out_meta[i]['error'] is not None:
                db_data = {'obs:obs_id': obs_id,
                           'error': out_meta[i]['error']}
                for gb, g in zip(group_by, out_meta[i]['group']):
                    db_data['dets:'+gb] = g

                if len(err_db.inspect(db_data)) == 0:
                    err_db.add_entry(db_data, filename=False)
...
def multilayer_load_and_preprocess(obs_id, configs_init, configs_proc,
    ...
    group_by_init, groups_init, error_init = get_groups(obs_id, configs_init,
                                                        context_init)
    group_by_proc, groups_proc, error_proc = get_groups(obs_id, configs_proc,
                                                        context_proc)
