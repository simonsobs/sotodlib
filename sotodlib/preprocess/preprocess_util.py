...
from sotodlib.core.util import load_h5
...
def cleanup_archive(configs, logger=None):
    ...
    if os.path.exists(configs['archive']['index']):
        ...
        if archive_files:
            ...
            with load_h5(latest_file, mode="r+") as f:
                keys = list(f.keys())
                for key in keys:
...
def cleanup_mandb(error, outputs, configs, logger=None, overwrite=False):
    ...
    if error is None:
        ...
        with load_h5(dest_file, mode='a') as f_dest:
            with load_h5(src_file, mode='r') as f_src:
