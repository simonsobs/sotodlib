...
def det_splits_relative(aman, det_left_right=False, det_upper_lower=False, det_in_out=False):
    ...
    if det_left_right:
        mask = xi <= xi_median
        aman.det_flags.wrap_dets('det_left', np.logical_not(mask))
        mask = xi > xi_median
        aman.det_flags.wrap_dets('det_right', np.logical_not(mask))
    if det_upper_lower:
        mask = eta <= eta_median
        aman.det_flags.wrap_dets('det_lower', np.logical_not(mask))
        mask = eta > eta_median
        aman.det_flags.wrap_dets('det_upper', np.logical_not(mask))
    if det_in_out:
