...
def highpass_filter(data, cutoff, fs, order=5):
    ...
    '''High-pass filtering used especially for SAT
...
def get_xieta_src_centred(ctime, az, el, data, sso_name, threshold=5):

    '''Create a planet centred coordinate system for single detector data
...
def define_fit_params(band, tele, return_beamsize=False):
    ...
    bounds_min = (0, -offset_bound, -offset_bound, fwhm_min, fwhm_min, 0., 1e-3)
...
def tod_sim_pointing(space_time_pointing, a, xi0, eta0):
    ...
    data_sim = gaussian2d(space_time_pointing['xieta'], 
                          a, xi0, eta0, fwhm_xi, fwhm_eta, phi)
    if tau is None:
        return data_sim
    data_sim_tau = tconst_convolve(space_time_pointing['time'], 
                                   data_sim, tau)
...
def tod_sim_beam(space_time_beam, fwhm_xi, fwhm_eta, phi):
    ...
    data_sim = gaussian2d(space_time_beam['xieta'], 
                          a, xi0, eta0, fwhm_xi, fwhm_eta, phi)
    if tau is None:
        return data_sim
    data_sim_tau = tconst_convolve(space_time_beam['time'], 
                                   data_sim, tau)
...
def fit_params(tod, ctime, az, el, band, sso_name, det_idx, highpass, cutoff, init_params):
    '''Function that fits indvidual time-streams and returns the parameters
    ...
    data -= np.mean(data)
    sample_rate = 1./np.mean(np.diff(tod.timestamps))
    ...
    if highpass and (cutoff is not None):
        ...
        # elegant way
        data[data<0] = 0 
    coord_transforms = get_xieta_src_centred(ctime, az, el, data, sso_name)
    ...
    space_time_pointing = {'xieta':xieta, 'time':ctime_band, 
                           'idx_main': idx_main_in_band,'fwhm_xi': p0[3], 
                           'fwhm_eta': p0[4], 'phi':p0[5], 'tau':p0[6]}


    p0_pointing = p0[:3]
    bounds_pointing = bounds[:, :3]
    try:
        popt_pointing, _ = curve_fit(tod_sim_pointing, space_time_pointing, 
                                     data_main, p0=p0_pointing, 
    ...
    q_delta = quat.rotation_xieta(popt_pointing[1], 
                                  popt_pointing[2]) # xi0 and eta0
    xi_t, eta_t,_ = quat.decompose_xieta(q_delta * q_t) #xi0 and eta0

    q_det = quat.rotation_xieta(xi_t, eta_t)
    xi_det_center, eta_det_center, psi_det_center = quat.decompose_xieta(~q_det * ~q_bore * q_obj)
    radius = (np.sqrt(xi_det_center**2 + eta_det_center**2))
    ...
    xieta = np.vstack((xi_band, eta_band))
    space_time_beam = {'xieta':xieta, 'time':ctime_band, 
                       'idx_main': idx_main_in_band,'a': p0[0], 
                       'xi_off': p0[1], 'eta_off':p0[2], 'tau':p0[6]}
    p0_beam = p0[3:6]
    bounds_beam = bounds[:, 3:6]
     
    try:
         popt_beam, _ = curve_fit(tod_sim_beam, space_time_beam, 
                                 data_main, p0=p0_beam, 
                                 bounds=bounds_beam,)
    except:
        all_params = np.empty(9)
        all_params[:] = np.nan
    ...
    try:
        popt, pcov = curve_fit(tod_sim_all, space_time, data_main, 
                               p0=p0, bounds=bounds,)
...
def make_maps(path, outdir, ofilename, sso_name, beamsize, n_modes):
    ...
    ctx = Context(opj(path,'context','context.yaml'))
    obs_id = ctx.obsdb.get()[0]['obs_id']
    tod = ctx.get_obs(obs_id)

    ofilename = opj(outdir, ofilename)
    mask_dict = {'shape': 'circle',
                 'xyr': (0, 0, 5*beamsize)}
   
    source_flags = planets.compute_source_flags(
    tod=tod, mask=mask_dict, center_on=sso_name,
    res=0.02)
    
    ## Make the maps in degrees
    ## mask ~ 5*beamsize
    ## map size and mask radius could be user inputs
    planets.make_map(tod, center_on=sso_name, scan_coords=True, 
                n_modes=n_modes, source_flags=source_flags, size=10*beamsize, 
                filename=ofilename,comps='T')



def run(indir, outdir, f_format, tube, band, wafer, sso_name, year, month, day, tele, highpass,
        cutoff, map_make, n_modes):
    ...
    file_name = 'calobs_%s_%s_%s_%s_%s_%s-%02d-%02d'%(tele, tube, band, wafer, sso_name, year, int(month), int(day))
    ...
    df = pd.DataFrame(columns=['amp','xi0','eta0','fwhm_xi','fwhm_eta','phi','tau', 'det_name', 'snr'], index=batch)
    

    for det in batch:
        params = fit_params(tod, ctime, az, el, band, sso_name, det, highpass, cutoff, init_params)
    ...
    if rank==0:
        ...
        new_dtypes = {"amp": np.float64, "xi0": np.float64, "eta0":np.float64, "fwhm_xi":np.float64, \
                      "fwhm_eta":np.float64, "phi":np.float64, "tau":np.float64, "det_name":str, "snr":np.float64}
        ...
        obs_id = '%s_%s_%s_%s_%s_%s-%02d-%02d'%(tele, tube, band, wafer, sso_name, year, int(month), int(day))
        ...
        ## Assert tables dependnecy is properly satisfied
...
def main():
    ...
    help='The location for the .h5 ouput files to be stored.')
    parser.add_argument('--f_format', action='store', dest='f_format',
    help='The format of the simulations [g3, h5, context].')
    parser.add_argument('--tele', action='store', dest='tele',
    help='The telescope name [LAT, SAT].')
    parser.add_argument('--tube', action='store', dest='tube',
    help='The tube name [c1, i6, o6, ...] for LAT, [SAT1, SAT2, ...] for SAT.')
    parser.add_argument('--band', action='store', dest='band', 
    help='Frequency band [30,40,90,150,230,290].')
    parser.add_argument('--wafer', action='store', dest='wafer',
    help='The wafer name [wafers number increases from the center and \
    anti-cloackwise] across the focalplane.')
    parser.add_argument('--sso_name', action='store', dest='sso_name',
    ...
    parser.add_argument('--highpass', action='store_true', dest='highpass',
    help='If True, use highpass butterworth filters (especiall useful for SATs).')
    parser.add_argument('--cutoff', action='store', dest='cutoff', default=None,
    help='The cutoff frequency to be used in the filtering.', type=float)
    #parser.add_argument('--sample_rate', action='store', dest='sample_rate', default=None, 
    #help='Sampling rate of the observation.', type=int)
    parser.add_argument('--map_make', action='store_true', dest='map_make',
    help='Make planet maps of the simulations fitted.')
    parser.add_argument('--n_modes', action='store', dest='n_modes', default=None, 
    help='Number of PCA modes to be removed in the planet mapmaker.', type=int) 

    args = parser.parse_args()
   
    run(args.indir, args.outdir, args.f_format, args.tube, args.band, args.wafer, args.sso_name, args.year, 
        args.month, args.day, args.tele, args.highpass, args.cutoff, args.map_make, args.n_modes)
