...
def calc_timeconstant(aman, hkdata, idxs=None, n_bins=40):
    ...
    for i_det, m in enumerate(det_mask):
        ...
        for filt_key in fit_result["fit_coadd"].keys():
            ...
            a0s = [
                fit_result["fit_coadd"][filt_key][f_key][i_det].best_values["a0"]
                if not isinstance(
                    fit_result["fit_coadd"][filt_key][f_key][i_det], (int, float)
                )
                else np.nan
                for f_key in fit_result["fit_coadd"][filt_key].keys()
            ]
            t0s = [
                fit_result["fit_coadd"][filt_key][f_key][i_det].best_values["t0"]
                if not isinstance(
                    fit_result["fit_coadd"][filt_key][f_key][i_det], (int, float)
                )
                else np.nan
            ...
            for fit_key in ["fit_amp", "fit_phase__fix_tau", "fit_phase__free"]:
                ...
                if fit_key == "fit_amp":
                    ...
                    if fit_key == "fit_phase__fix_tau":
                        if not isinstance(
                            fit_result["fit_amp"][filt_key][i_det], (int, float)
                        ):
                            params["tau"].set(
...
def get_fit_params(cal_type):
    ...
    if cal_type == "gain":
        ...
        for i, a in enumerate(amps_init):
            params_base["fit_coadd"].add(f"a{i}", value=a)
            params_base["fit_coadd"].add(f"t{i}", value=0.5, min=0, max=1)
