...
def plot_hkdata(aman, hkdata, cal_type, show=True, output_dir=None):
    ...
    # Heater temperature
    axes["D"].plot(
        data_temp["heater"]["t"], data_temp["heater"]["temp"], "-", label="heater"
    )
...
def plot_tod(aman, i_det, cal_type, show=True, output_dir=None):
    ...
    if cal_type == "gain":
        ...
        fig.suptitle(
            f"Stimulator data, {ufm}, i_det: {i_det}, det_id: {aman.det_info.det_id[i_det]}"
        )
        ...
        axes[i_y, i_x].legend()
        axes[i_y, i_x].set_ylim(
            min(aman.signal_hpf[i_det][100:-100]), max(aman.signal_hpf[i_det][100:-100])
        )
        if ufm[0] == "M":
        ...
        axes[i_y, i_x].errorbar(
            x, y, yerr, fmt="o", capsize=5, color="C2", zorder=0, label="(HPF+LPF)ed"
        )
        axes[i_y, i_x].set_xlabel("Timing (1 cycle)")
        axes[i_y, i_x].set_ylabel("TOD ave [pW]")

        a0s = [
            aman.stm_cal["fit_coadd"]["hpf"]["f1_gain"][f"a{i_freq}"][i_det]
            for i_freq in range(7)
        ]
        t0s = [
            aman.stm_cal["fit_coadd"]["hpf"]["f1_gain"][f"t{i_freq}"][i_det]
            for i_freq in range(7)
        ]
        axes[i_y, i_x].plot(x, func_sines(x, *a0s, *t0s), "-", color="red", zorder=1)
        a0s = [
            aman.stm_cal["fit_coadd"]["lpf"]["f1_gain"][f"a{i_freq}"][i_det]
            for i_freq in range(7)
        ]
        t0s = [
            aman.stm_cal["fit_coadd"]["lpf"]["f1_gain"][f"t{i_freq}"][i_det]
            for i_freq in range(7)
        ]
        axes[i_y, i_x].plot(x, func_sines(x, *a0s, *t0s), "-", color="green", zorder=1)
        y = aman.stm_cal["fit_coadd"]["hpf"]["f1_gain"]["a0"][i_det] * np.sin(
            (x - aman.stm_cal["fit_coadd"]["hpf"]["f1_gain"]["t0"][i_det]) * 2 * np.pi
        )
        axes[i_y, i_x].plot(
            x,
            y,
        ...
        i_y = 2
        i_x = 0
        hpf = tod_ops.filters.high_pass_sine2(
            aman.stm_cal.filtering_params["hpf_cutoff"]
        )
        filter_cutoff = (
        ...
        lpf = tod_ops.filters.low_pass_sine2(
            filter_cutoff,
            filter_cutoff * aman.stm_cal.filtering_params["lpf_width_fraction"],
        )
        ...
        fig.suptitle(
            f"Stimulator data, i_det: {i_det}, det_id: {aman.det_info.det_id[i_det]}"
        )
        ...
        axes[i_y, i_x].plot(
            ...
            func_response_amplitude(
                f,
                tau=aman.stm_cal["fit_amp"]["lpf"]["tau"][i_det],
                a=aman.stm_cal["fit_amp"]["lpf"]["a"][i_det],
            ),
            "-",
            color="red",
            zorder=3,
            label=rf"$\tau$= {aman.stm_cal['fit_amp']['lpf']['tau'][i_det] * 1e3:.2f}ms",
        )
        axes[i_y, i_x].plot(
            f,
            func_response_amplitude(
                f,
                tau=aman.stm_cal["fit_amp"]["lpf"]["tau"][i_det] * 1.1,
                a=aman.stm_cal["fit_amp"]["lpf"]["a"][i_det],
            ),
            "--",
            color="orange",
            zorder=3,
            label=r"$\pm 10\%$ change of $\tau$",
        )
        axes[i_y, i_x].plot(
            f,
            func_response_amplitude(
                f,
                tau=aman.stm_cal["fit_amp"]["lpf"]["tau"][i_det] * 0.9,
                a=aman.stm_cal["fit_amp"]["lpf"]["a"][i_det],
            ),
            "--",
            color="orange",
        ...
        axes[i_y, i_x].plot(
            f,
            func_response_phase_with_dt(f, tau, theta_geo, dt),
            "-",
            color="blue",
            zorder=3,
        ...
        for i_freq, f_key in enumerate(["f1", "f2", "f3", "f4", "f5", "f6", "f7"]):
            ...
            a0s = [
                aman.stm_cal["fit_coadd"]["hpf"][f_key][f"a{i_freq}"][i_det]
                for i_freq in range(7)
            ]
            t0s = [
                aman.stm_cal["fit_coadd"]["hpf"][f_key][f"t{i_freq}"][i_det]
                for i_freq in range(7)
            ]
            axes[i_y, i_x].plot(
                x, func_sines(x, *a0s, *t0s), "-", color="red", zorder=5
            )
            a0s = [
                aman.stm_cal["fit_coadd"]["lpf"][f_key][f"a{i_freq}"][i_det]
                for i_freq in range(7)
            ]
            t0s = [
                aman.stm_cal["fit_coadd"]["lpf"][f_key][f"t{i_freq}"][i_det]
                for i_freq in range(7)
            ]
            axes[i_y, i_x].plot(
                x, func_sines(x, *a0s, *t0s), "-", color="green", zorder=5
            )
            y = aman.stm_cal["fit_coadd"]["hpf"][f_key]["a0"][i_det] * np.sin(
                (x - aman.stm_cal["fit_coadd"]["hpf"][f_key]["t0"][i_det]) * 2 * np.pi
            )
            axes[i_y, i_x].plot(
                x,
                y,
