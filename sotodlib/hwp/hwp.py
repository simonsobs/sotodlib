...
def extract_hwpss(aman, hwp_angle=None, mode=0, 
                  name='hwpss_ext', signal='signal',
...
def demod(aman, hwp_angle=None, bpf_width=0.5, lpf_cut=0.5, signal='signal'):
    ...
    ** Simple demoduation function **
    ...
    aman.wrap('signal_demod_prelfilt', 
              tod_ops.fourier_filter(aman, prelfilt, 
                                     detrend=None, 
                                     signal_name=signal), 
              [(0, 'dets'), (1, 'samps')])
    prehfilt = tod_ops.filters.high_pass_butter4(fc=4 * speed - bpf_width)
    aman.wrap('signal_demod_prebfilt', 
              tod_ops.fourier_filter(aman, prehfilt, 
                                     detrend=None, 
                                     signal_name='signal_demod_prelfilt'), 
              [(0, 'dets'), (1, 'samps')])

    aman.wrap('signal_demod_wo_lfilt', 
              (aman.signal_demod_prebfilt * np.exp(-1j * (4 * aman.hwp_angle))).real, 
              [(0, 'dets'), (1, 'samps')])
    lfilt = tod_ops.filters.low_pass_butter4(fc=lpf_cut)
    aman.wrap('signal_demod', 
