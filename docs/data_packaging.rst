...
Every SO Level 2 DAQ node runs the same six scripts, on a timer, in a fixed
order.  This page describes what each one does, the databases and
configuration they share, and what to do when one of them fails.
...
.. code-block:: none
    ...
    /prefix/hk/<timecode>/       ---->  G3tHk (hk db)        <---- update_g3thk_database
      *.g3 from OCS aggregator

    /prefix/smurf/<timecode>/    ---->  G3tSmurf (lvl2 db)   <---- update_g3tsmurf_db
    /prefix/timestreams/<tc>/           files, observations,
...
The ordering matters and is enforced by status checks, not by the scheduler:
``make_book`` only touches ``UNBOUND`` Books, ``update_librarian`` only
touches ``BOUND`` Books, and ``cleanup_level2`` refuses to delete anything
that is not at least ``UPLOADED``.  Running them out of order is safe; it just
...
laid out in timecode directories::
...
:meth:`sotodlib.io.imprinter.Imprinter.get_book_path`::
...
node, in one-to-one correspondence.
...
``DATAPKG_ENV`` environment variable.  This is what lets the same
site-pipeline-configs checkout work on the site machines, on simons1, and on
a laptop.
...
A config file then refers to those tags::
    ...
    g3tsmurf_db: "{metadata}/g3tsmurf.db"

If a config uses a tag that the environment file does not define,
``load_configs`` raises a ``ValueError`` naming the missing tag.

``DATAPKG_ENV`` must define ``configs`` for
:meth:`Imprinter.for_platform <sotodlib.io.imprinter.Imprinter.for_platform>`
(and therefore for ``cleanup_level2`` and ``imprinter-cli``) to work; it
resolves the config path as ``<configs>/<platform>/imprinter.yaml``.
...
.. code-block:: yaml
    ...
    data_prefix: "/path/to/daq-node/"   # contains hk/, smurf/, timestreams/
    g3tsmurf_db: "/path/to/g3tsmurf.db"
    g3thk_db: "/path/to/g3hk.db"

    finalization:
        servers:
            - smurf-suprsync: "smurf-sync-so1"       # OCS instance-id
              timestream-suprsync: "timestream-sync-so1"
              pysmurf-monitor: "monitor-so1"
            - smurf-suprsync: "smurf-sync-so2"
              timestream-suprsync: "timestream-sync-so2"
              pysmurf-monitor: "monitor-so2"

The ``finalization`` block and ``g3thk_db`` are what make
:ref:`finalization <dpkg-finalization>` work; without them the pipeline
cannot tell whether Level 2 data transfer is complete.  ``G3tHk`` derives its
archive path as ``<data_prefix>/hk`` and takes its list of instance-ids from
...
.. code-block:: yaml
    ...
    db_path: /path/to/imprinter.db        # the book database
    daq_node: satp1                       # names hk/smurf/stray books & dirs
    g3tsmurf: /path/to/g3tsmurf_config.yaml
    output_root: /abs/path/to/staging     # where bound books are written
    librarian_conn: satp1-librarian       # key in the hera_librarian client settings
...
* Each tel tube becomes its own set of ``obs``/``oper`` Books and its own
  output directory.  SATs have one entry; the LAT has many.
...
``build_hk`` and ``build_det`` let one node handle only housekeeping (e.g. a
site-wide HK aggregator) or only detector data.
...
.. note::
    ...
    *positional* argument and ``cleanup_level2`` takes a platform name
    instead.  This inconsistency is historical.
...
thousands of error lines, and then re-raises the first failure so the
...
.. note::
    ...
    from Python is not equivalent to running the script with no arguments.
    Pass the value explicitly if it matters.
...
``FAILED`` Books.  Set ``DATAPKG_ENV`` first, and run from an account with
write access to both the database and ``output_root``::
